...
const YEAR = 2026;
const TARGET_KEY = 'target_2026' as const;
...
export default function Achievements2026() {
        ...
        <div className="relative max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold text-foreground mb-3">
              {t(`أبرز ما تم تحقيقه في ${YEAR}`, `Key Highlights of ${YEAR}`)}
            </h2>
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
              {t('نظرة سريعة على أهم الإنجازات الاستراتيجية التي تحققت خلال العام', 'A quick look at the most important strategic achievements of the year')}
            </p>
          </motion.div>
            ...
            {(() => {
              ...
              const highlightItems = (settings?.highlights && settings.highlights.length > 0)
                ? settings.highlights.map((h: any, i: number) => ({
                    ...
                    titleAr: h.titleAr || '', titleEn: h.titleEn || '',
                    descAr: h.descAr || '', descEn: h.descEn || '',
              ...
              return highlightItems.map((item, i) => (
                <motion.div key={i} custom={i} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}
                  ...
                  className="group relative rounded-2xl p-6 border backdrop-blur-sm cursor-default"
            ...
            {pillarStats.map((ps, i) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && <motion.div layoutId="activeTab2026" className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-1 rounded-full" style={{ background: ps.pillar.color }} />}
                </motion.button>
              );
            })}
          </div>
              ...
              >
                <div className="p-6 flex items-center gap-4" style={{ borderBottom: `3px solid ${activePS.pillar.color}` }}>
                  ...
                  <div className="flex gap-4 text-center">
                    {activePS.completed.length > 0 && (
                      <div className="px-4 py-2 rounded-lg" style={{ background: `${activePS.pillar.color}10` }}>
                        <div className="text-2xl font-bold" style={{ color: activePS.pillar.color }}>{activePS.completed.length}</div>
                        <div className="text-[10px] text-muted-foreground">{t('مكتمل', 'Completed')}</div>
                      </div>
                    )}
                    {activePS.launched.length > 0 && (
                      <div className="px-4 py-2 rounded-lg bg-accent">
                        <div className="text-2xl font-bold text-accent-foreground">{activePS.launched.length}</div>
                        <div className="text-[10px] text-muted-foreground">{t('منطلق', 'Launched')}</div>
                      </div>
                    )}
                    {activePS.delayed.length > 0 && (
                      <div className="px-4 py-2 rounded-lg bg-red-50 dark:bg-red-950/30">
                        <div className="text-2xl font-bold text-red-600">{activePS.delayed.length}</div>
                        <div className="text-[10px] text-muted-foreground">{t('متأخر', 'Delayed')}</div>
                      </div>
                    )}
                    {activePS.kpisAchieved.length > 0 && (
                      <div className="px-4 py-2 rounded-lg bg-green-50 dark:bg-green-950/30">
                        <div className="text-2xl font-bold text-green-600">{activePS.kpisAchieved.length}</div>
                        <div className="text-[10px] text-muted-foreground">{t('مؤشر محقق', 'Achieved')}</div>
                      </div>
                    )}
                    {activePS.kpisNotAchieved.length > 0 && (
                      <div className="px-4 py-2 rounded-lg bg-red-50 dark:bg-red-950/30">
                        <div className="text-2xl font-bold text-red-500">{activePS.kpisNotAchieved.length}</div>
                        <div className="text-[10px] text-muted-foreground">{t('مؤشر لم يتحقق', 'Not Achieved')}</div>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {(activePS.completed.length > 0 || activePS.launched.length > 0 || activePS.delayed.length > 0) && (
                  <div className="p-6 pb-3">
                    <h4 className="text-sm font-semibold text-muted-foreground mb-3 flex items-center gap-2">
                      <Rocket className="h-4 w-4" /> {t('المشاريع', 'Projects')}
                    </h4>
        ...
        <div className="max-w-4xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-white/70 text-lg max-w-2xl mx-auto mb-8">
              {t(
                settings?.footer_subtitle_ar || 'نواصل المسيرة نحو تحقيق رؤيتنا الاستراتيجية بخطى واثقة وإنجازات ملموسة',
                settings?.footer_subtitle_en || 'We continue our journey towards achieving our strategic vision with confident steps and tangible results'
              )}
            </p>
