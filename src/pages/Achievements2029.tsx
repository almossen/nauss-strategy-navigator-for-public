...
const YEAR = 2029;
const TARGET_KEY = 'target_2029' as const;
...
export default function Achievements2029() {
            ...
            {pillarStats.map((ps, i) => {
              ...
              return (
                <motion.button key={ps.pillar.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  {isActive && <motion.div layoutId="activeTab2029" className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-1 rounded-full" style={{ background: ps.pillar.color }} />}
                </motion.button>
              );
            })}
          </div>
